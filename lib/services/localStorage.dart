...
class LocalStorage{
  ...
  Future<Plate> plate(num id)async{
    final db = await database;
    return Plate.fromJson(db.query('plates', where: '"id" = $id') as Map<String, dynamic>);
  }

  Future<List> platesOffer() async {
    final db = await database;
    final List<Map<String, dynamic>> maps = await db.query('plates',where: '"offerPrice" > 0');
    print(maps);
    return maps;
  ...
  Future<List> bestPlates() async {
    ...
    final List<Map<String, dynamic>> maps = await db.rawQuery('SELECT * FROM "plates" ORDER BY "rating" DESC,"price" DESC LIMIT 3',);
    return maps;
  }
}